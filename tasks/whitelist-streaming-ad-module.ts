...
task('whitelist-streaming-ad-module', 'whitelists the new reference module StreamingAdModule from madfi').setAction(async ({}, hre) => {
  ...
  const MODULE_ADDRESS = addrs['StreamingAdModule'];
  ...
  console.log(`\n\t-- Whitelisting Reference Module (at: ${MODULE_ADDRESS}) --`);
  await waitForTx(
    lensHub.whitelistReferenceModule(MODULE_ADDRESS, true)
  );
});
